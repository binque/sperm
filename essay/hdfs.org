...
file:./images/hdfs-ha-namenode.png
...
** Usage
*** file system API
因为hdfs本质上是一个文件系统，只不过这个文件系统是应用层面上的。获得这个文件系统之后很多操作就很明了了，因为很多概念是可以直接从linux文件系统借鉴过来的。
   1. 首先获取HDFS配置。
   2. 然后根据配置创建FileSystem.这个是一个通用接口可以替换成为本地文件。
   3. FileSystem提供了一系列访问方法。
#+BEGIN_SRC Java
