...
**** 安装配置
   ...
   - 下载hadoop-lzo. git clone git://github.com/kevinweil/hadoop-lzo.git https://github.com/kevinweil/hadoop-lzo
...
**** 编写proto文件
首先创建proto文件，比如segment_mark.proto
#+BEGIN_EXAMPLE
package com.umeng.analytics.proto;
...
option java_outer_classname = "SegmentMarkProtos1";
...
使用protoc --java_out=<dir> segment_mark.proto就会在<dir>下面生成SegmentMarkProtos1.java文件。
...
此外我们还需要为这个类写几个辅助类，但是索性的是辅助类并不是很复杂。
   ...
   - Writable
#+BEGIN_SRC Java
public class SegmentMarkLzoProtobufBlockInputFormat extends LzoProtobufBlockInputFormat<SegmentMark> {
  public SegmentMarkLzoProtobufBlockInputFormat() {
    super(new TypeRef<SegmentMark>(){});
  }
}

public class SegmentMarkLzoProtobufBlockOutputFormat extends LzoProtobufBlockOutputFormat<SegmentMark> {
  public SegmentMarkLzoProtobufBlockOutputFormat() {
    super(new TypeRef<SegmentMark>(){});
  }
}

public class SegmentMarkProtobufWritable extends ProtobufWritable<SegmentMark> {
  public SegmentMarkProtobufWritable() {
    super(new TypeRef<SegmentMark>(){});
  }
  public SegmentMarkProtobufWritable(SegmentMark m) {
    super(m, new TypeRef<SegmentMark>(){});
  }
}
#+END_SRC
...
**** 使用lzo
做好上面的工作之后就可以开始使用了。
   - job.setInputFormatClass(SegmentMarkLzoProtobufBlockInputFormat.getInputFormatClass(SegmentMark.class, job.getConfiguration()));
   - job.setOutputFormatClass(SegmentMarkLzoProtobufBlockOutputFormat.class);
#+BEGIN_SRC Java
    public static class XMap
            extends
            Mapper<LongWritable, ProtobufWritable<SegmentMark>, Text, ProtobufWritable<SegmentMark>> {
        @Override
        protected void map(LongWritable key,
                ProtobufWritable<SegmentMark> value, Context context)
                throws IOException, InterruptedException {
            SegmentMark sm = value.get();
            String appKey = sm.getAppKey();
            String umid = sm.getUmid();
            context.write(new Text(appKey + "_" + umid), value);
        }
    }

    public static class Reduce
            extends
            Reducer<Text, ProtobufWritable<SegmentMark>, NullWritable, ProtobufWritable<SegmentMark>> {
        @Override
        protected void reduce(Text key,
                Iterable<ProtobufWritable<SegmentMark>> values, Context context)
                throws IOException, InterruptedException {
            for (ProtobufWritable<SegmentMark> psm : values) {
                psm.setConverter(SegmentMark.class);
                SegmentMark sm = psm.get();
            }
            SegmentMark.Builder sm = null;
            sm = SegmentMark.newBuilder();
            sm.setAppKey(appKey);
            sm.setUmid(umid);
            sm.setTag(SegmentMark.Tag.ALL);
            SegmentMark mark = sm.build();
            info(TextFormat.printToString(mark));
            context.write(null, new SegmentMarkProtobufWritable(mark));
        }
    }
#+END_SRC
*NOTE（dirlt）：这里面代码不正确，只是示例*
...
多路输入分为三种情况，分别是：
   ...
   - 多路htable
   - 多路hdfs和htable混合。
第三中情况包含前面两种情况，所以下面看看第三种情况如何实现。

*NOTE(dirlt):现在看来只是支持了多路hdfs输入以及1路的htable输入*

实现多路输入[[file:mapred.org][非常简单]]， 但是过程非常magic.：
...
*TODO（dirlt）：至于为什么这样可以就需要阅读源代码，这也意味着不保证其他hadoop版本也可以这么做*

*** 多路输出
多路输出代码可以参考[[file:mapred.org][这里]] 其中一小节"多路输出"
   - 使用MultipleOutputs 多文件和table混合输出（和mrunit配合不好）
   - 使用MultipleTableOutputFormat 多table输出
   - 使用MultipleOutputFormat 多文件输出
     - 需要继承这个类并且复写一些方法，然后设置OutputFormat为这个类
     - 似乎高版本被MultipleOutputs代替
