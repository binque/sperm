...
*** 多路输出
多路输出代码可以参考[[file:mapred.org][这里]] 其中一小节"验证多路输出正确性",主要步骤是这样的：
   - 使用FileOutputFormat设置好输出文件目录
   - 使用TableOutputFormat设置好输出table
   - 设置MultipleOutputs参数，使用命名前缀来进行区分。
   - 在mapper或者是reducer里面创建MultipleOutputs对象，按照命名前缀来做多路输出。
