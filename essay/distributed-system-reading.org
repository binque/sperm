...
*NOTE（dirlt）：说实话没看懂他们到底是什么意思* 不过似乎他们在解决文件数量这个问题上面使用的是分布式master解决方案，每个master不会持有所有的数据。分布式master的解决方案可以让每个master保存100 millions文件数目，并且以1MB为blocksize. 但是1MB已经是blocksize能够缩小到的底线了，如果低于1MB的话那么就需要重新设计。
